...
export function useInputInterface(initial, title) {
    const [text, setText] = useState(initial);
    const textRef = useRef(text);
    useEffect(() => {
        textRef.current = text;
    }, [text]);
    const { componentId, addAction } = useComponentInterface('TextInput', 'Input field with assist');
    const actions = useRef({ get: '', update: '' });
    useEffect(() => {
        actions.current.get = addAction('getText', 'Return text', () => textRef.current);
        actions.current.update = addAction('setText', 'Set text', ({ text }) => setText(text));
    }, [addAction]);
